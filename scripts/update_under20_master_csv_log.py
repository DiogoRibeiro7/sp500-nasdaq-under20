...
def _load_existing_master() -> pd.DataFrame:
    ...
    if "Name" not in df.columns:
        log.debug("Existing master CSV has no Name column — adding it for backwards compatibility.")
        df["Name"] = pd.NA
...
def _fetch_names_from_yfinance(
    ...
    Uses ``yf.Tickers`` so that yfinance can parallelise HTTP requests
    internally, avoiding the serial-per-ticker bottleneck of the old
    ``yf.Ticker(t).info`` loop.
    """
    names: Dict[str, str] = {}
